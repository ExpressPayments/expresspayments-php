 ...
 * <a href="https://expressplatby.cz/docs/api#products">Products</a> help you track inventory or provisioning, and prices help you track payment terms. Different physical goods or levels of service should be represented by products, and pricing options should be represented by prices. This approach lets you change prices without having to change your provisioning scheme.
 *
 * For example, you might have a single &quot;gold&quot; product that has prices for $10/month, $100/year, and €9 once.
 *
 * Related guides: <a href="https://expressplatby.cz/docs/billing/subscriptions/set-up-subscription">Set up a subscription</a>, <a href="https://expressplatby.cz/docs/billing/invoices/create">create an invoice</a>, and more about <a href="https://expressplatby.cz/docs/products-prices/overview">products and prices</a>.
 *
 ...
 * @property null|\ExpressPlatby\ExpressPlatbyObject $currency_options Prices defined in each available currency option. Each key must be a three-letter <a href="https://www.iso.org/iso-4217-currency-codes.html">ISO currency code</a> and a <a href="https://expressplatby.cz/docs/currencies">supported currency</a>.
 * @property null|\ExpressPlatby\ExpressPlatbyObject $custom_unit_amount When set, provides configuration for the amount to be adjusted by the customer during Checkout Sessions and Payment Links.
 ...
 * @property string|\ExpressPlatby\Product $product The ID of the product this price is associated with.
 * @property null|\ExpressPlatby\ExpressPlatbyObject $recurring The recurring components of a price such as <code>interval</code> and <code>usage_type</code>.
 * @property null|string $tax_behavior Only required if a <a href="https://expressplatby.cz/docs/tax/products-prices-tax-categories-tax-behavior#setting-a-default-tax-behavior-(recommended)">default tax behavior</a> was not provided in the ExpressPlatby Tax settings. Specifies whether the price is considered inclusive of taxes or exclusive of taxes. One of <code>inclusive</code>, <code>exclusive</code>, or <code>unspecified</code>. Once specified as either <code>inclusive</code> or <code>exclusive</code>, it cannot be changed.
 * @property null|\ExpressPlatby\ExpressPlatbyObject[] $tiers Each element represents a pricing tier. This parameter requires <code>billing_scheme</code> to be set to <code>tiered</code>. See also the documentation for <code>billing_scheme</code>.
 ...
 * @property null|\ExpressPlatby\ExpressPlatbyObject $transform_quantity Apply a transformation to the reported usage or set quantity before computing the amount billed. Cannot be combined with <code>tiers</code>.
...
class Price extends ApiResource
{
     ...
     *
     * @return \ExpressPlatby\SearchResult<\ExpressPlatby\Price> the price search results
