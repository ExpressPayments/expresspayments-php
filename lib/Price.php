 ...
 * <a href="https://docs.epayments.network/api#products">Products</a> help you track inventory or provisioning, and prices help you track payment terms. Different physical goods or levels of service should be represented by products, and pricing options should be represented by prices. This approach lets you change prices without having to change your provisioning scheme.
 *
 * For example, you might have a single &quot;gold&quot; product that has prices for $10/month, $100/year, and €9 once.
 *
 * Related guides: <a href="https://docs.epayments.network/billing/subscriptions/set-up-subscription">Set up a subscription</a>, <a href="https://docs.epayments.network/billing/invoices/create">create an invoice</a>, and more about <a href="https://docs.epayments.network/products-prices/overview">products and prices</a>.
 *
 ...
 * @property null|\ExpressPayments\ExpressPaymentsObject $currency_options Prices defined in each available currency option. Each key must be a three-letter <a href="https://www.iso.org/iso-4217-currency-codes.html">ISO currency code</a> and a <a href="https://docs.epayments.network/currencies">supported currency</a>.
 * @property null|\ExpressPayments\ExpressPaymentsObject $custom_unit_amount When set, provides configuration for the amount to be adjusted by the customer during Checkout Sessions and Payment Links.
 ...
 * @property string|\ExpressPayments\Product $product The ID of the product this price is associated with.
 * @property null|\ExpressPayments\ExpressPaymentsObject $recurring The recurring components of a price such as <code>interval</code> and <code>usage_type</code>.
 * @property null|string $tax_behavior Only required if a <a href="https://docs.epayments.network/tax/products-prices-tax-categories-tax-behavior#setting-a-default-tax-behavior-(recommended)">default tax behavior</a> was not provided in the ExpressPayments Tax settings. Specifies whether the price is considered inclusive of taxes or exclusive of taxes. One of <code>inclusive</code>, <code>exclusive</code>, or <code>unspecified</code>. Once specified as either <code>inclusive</code> or <code>exclusive</code>, it cannot be changed.
 * @property null|\ExpressPayments\ExpressPaymentsObject[] $tiers Each element represents a pricing tier. This parameter requires <code>billing_scheme</code> to be set to <code>tiered</code>. See also the documentation for <code>billing_scheme</code>.
 ...
 * @property null|\ExpressPayments\ExpressPaymentsObject $transform_quantity Apply a transformation to the reported usage or set quantity before computing the amount billed. Cannot be combined with <code>tiers</code>.
...
class Price extends ApiResource
{
     ...
     *
     * @return \ExpressPayments\SearchResult<\ExpressPayments\Price> the price search results
