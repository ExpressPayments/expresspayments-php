 ...
 * initially charged.
 *
 * Related guide: <a href="https://docs.epayments.network/refunds">Refunds</a>
 *
 ...
 * @property null|string|\ExpressPayments\Charge $charge ID of the charge that's refunded.
 ...
 * @property null|string|\ExpressPayments\BalanceTransaction $failure_balance_transaction After the refund fails, this balance transaction describes the adjustment made on your account balance that reverses the initial balance transaction.
 ...
 * @property null|\ExpressPayments\ExpressPaymentsObject $next_action
 * @property null|string|\ExpressPayments\PaymentIntent $payment_intent ID of the PaymentIntent that's refunded.
 * @property null|string $reason Reason for the refund, which is either user-provided (<code>duplicate</code>, <code>fraudulent</code>, or <code>requested_by_customer</code>) or generated by ExpressPayments internally (<code>expired_uncaptured_charge</code>).
 * @property null|string $receipt_number This is the transaction number that appears on email receipts sent for this refund.
 * @property null|string|\ExpressPayments\TransferReversal $source_transfer_reversal The transfer reversal that's associated with the refund. Only present if the charge came from another ExpressPayments account.
 * @property null|string $status Status of the refund. For credit card refunds, this can be <code>pending</code>, <code>succeeded</code>, or <code>failed</code>. For other types of refunds, it can be <code>pending</code>, <code>requires_action</code>, <code>succeeded</code>, <code>failed</code>, or <code>canceled</code>. Learn more about <a href="https://docs.epayments.network/refunds#failed-refunds">failed refunds</a>.
 * @property null|string|\ExpressPayments\TransferReversal $transfer_reversal This refers to the transfer reversal object if the accompanying transfer reverses. This is only applicable if the charge was created using the destination parameter.
 */
class Refund extends ApiResource
{
     ...
     *
     * @return \ExpressPayments\Refund the canceled refund
