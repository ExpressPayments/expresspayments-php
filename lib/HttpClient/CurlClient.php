...
use ExpressPayments\Exception;
use ExpressPayments\ExpressPayments;
use ExpressPayments\Util;
...
class CurlClient implements ClientInterface, StreamingClientInterface
{
    ...
    /** @var \ExpressPayments\Util\RandomGenerator */
     ...
     * @param null|\ExpressPayments\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\ExpressPayments\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (('post' === $method) && (ExpressPayments::$maxNetworkRetries > 0)) {
        ...
        $opts[\CURLOPT_CAINFO] = ExpressPayments::getCABundlePath();
        if (!ExpressPayments::getVerifySslCerts()) {
        ...
        // force IPv4 resolving as ExpressPayments' API servers are only accessible over
        // IPv4 (see. https://github.com/expresspayments/expresspayments-php/issues/1045).
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            $msg = "Could not connect to ExpressPayments ({$url}).  Please check your "
                 . 'internet connection and try again.  If this problem persists, '
                . "you should check ExpressPayments' service status at "
                . 'https://twitter.com/expresspaymentsstatus, or';

                break;

            case \CURLE_SSL_CACERT:
            case \CURLE_SSL_PEER_CERTIFICATE:
            $msg = "Could not verify ExpressPayments' SSL certificate.  Please make sure "
            ...
            default:
                $msg = 'Unexpected error communicating with ExpressPayments.  '
                 . 'If this problem persists,';
        }
        $msg .= ' let us know at support@epayments.network.';
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\ExpressPayments\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= ExpressPayments::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['ep-should-retry'])) {
            if ('false' === $rheaders['ep-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['ep-should-retry']) {
                return true;
            }
        }
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            ExpressPayments::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            ExpressPayments::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(ExpressPayments::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= ExpressPayments::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which ExpressPayments use.
