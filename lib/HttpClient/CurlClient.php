...
use ExpressPlatby\Exception;
use ExpressPlatby\ExpressPlatby;
use ExpressPlatby\Util;
...
class CurlClient implements ClientInterface, StreamingClientInterface
{
    ...
    /** @var \ExpressPlatby\Util\RandomGenerator */
     ...
     * @param null|\ExpressPlatby\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\ExpressPlatby\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (('post' === $method) && (ExpressPlatby::$maxNetworkRetries > 0)) {
        ...
        $opts[\CURLOPT_CAINFO] = ExpressPlatby::getCABundlePath();
        if (!ExpressPlatby::getVerifySslCerts()) {
        ...
        // force IPv4 resolving as ExpressPlatby's API servers are only accessible over
        // IPv4 (see. https://github.com/expressplatby/expressplatby-php/issues/1045).
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to ExpressPlatby ({$url}).  Please check your "
                 . 'internet connection and try again.  If this problem persists, '
                 . "you should check ExpressPlatby's service status at "
                 . 'https://twitter.com/expressplatbystatus, or';

                break;

            case \CURLE_SSL_CACERT:
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify ExpressPlatby's SSL certificate.  Please make sure "
            ...
            default:
                $msg = 'Unexpected error communicating with ExpressPlatby.  '
                 . 'If this problem persists,';
        }
        $msg .= ' let us know at support@expressplatby.cz.';
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\ExpressPlatby\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= ExpressPlatby::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['expressplatby-should-retry'])) {
            if ('false' === $rheaders['expressplatby-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['expressplatby-should-retry']) {
                return true;
            }
        }
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            ExpressPlatby::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            ExpressPlatby::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(ExpressPlatby::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= ExpressPlatby::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which ExpressPlatby use.
